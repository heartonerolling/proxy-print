(function () {
  ...
  function stripDeckMakerMobileIntroUrls(urls) {
    // 例：スマホ表示で [A,B,C,D,...,J,A,A,A,A,B,B,B...] のように、
    ...
    // 前置きブロックはだいたい6〜25枚程度の「ほぼユニーク」な並びになる。
    // 1〜2枚目で同カードが続く本物のデッキ枚数とは区別する。
    if (repeatStart < 6 || repeatStart > 25) return urls;
    ...
    if (uniquePrefixCount / repeatStart < 0.8) return urls;

    // repeatStart以降に、前置きのカードが複数回まとまって現れていれば、
    // そこからが実際の枚数展開済みリストとみなす。
    const prefixSet = new Set(prefix);
    const nextWindow = keys.slice(repeatStart, repeatStart + Math.min(18, keys.length - repeatStart));
  ...
  function setStatus(text, isError = false) {
    console.log('[Deck → GitHub A4 Android bookmarklet v1.1]', text);
